...
一键批量下载 **Bilibili + YouTube** 视频字幕，输出为 Markdown，并可调用 **DeepSeek V4** 自动生成四层深度内容分析。

---

## 一句话定位

从 Bilibili / YouTube 批量下载视频字幕为 Markdown，支持 Ctrl+点击在 Obsidian 中直接打开，并可 AI 生成四层深度分析（拓扑结构/语义提取/认知机制/批判性重构）。
...
| **循环下载模式** | 下载完成后不退出，继续等待新链接输入，按 `q` 才退出 |
| **批量并发下载** | 一次性处理多个视频，充分利用带宽，大幅提升效率 |
| **智能语言选择** | 自动按优先级选择字幕语言（中文 > 英文 > 其他），减少手动配置 |
| **Obsidian 一键跳转** | 下载结果表格支持 Ctrl+点击，直接在 Obsidian 中打开文件 |
| **DeepSeek V4 分析** | 下载完成后可选择生成四层深度分析（拓扑结构/语义提取/认知机制/批判性重构） |
| **双语字幕翻译** | 下载完成后可选择生成英中双语字幕（原文 + > 译文格式） |
...
### 步骤四：配置 DeepSeek API（如需 AI 分析功能）

DeepSeek API 已支持 V4 系列模型，配置方式：
...
| **AI 分析** | DeepSeek V4 Pro | 四层深度分析生成（拓扑/语义/认知/批判） |
| **AI 翻译** | DeepSeek V4 Flash | 英中双语字幕翻译 |
...
2. 确认 `DEEPSEEK_MODEL` 使用的是 `deepseek-v4-pro` 或 `deepseek-v4-flash`
3. API 请求 timeout 已设置为 180 秒，如仍超时可能是网络问题

**Q: 下载的字幕文件名被截断或包含乱码字符？**
A: 程序会自动删除 Windows 文件名保留字符（`\\/:*?"<>|`）。如遇特殊字符问题，程序会直接删除而非替换。文件名最大长度限制为 150 字符。
...
### v0.4.0
- 新增 **双语字幕翻译** 功能，使用 Flash 模型快速翻译英文字幕
...
- 更新 **DeepSeek API** 至 V4 系列（`deepseek-v4-pro` / `deepseek-v4-flash`）
