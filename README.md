...
A: v0.4.2 已修复。旧版本的 `add_translation_to_file()` 把 `---` 误当成 YAML frontmatter 成对边界来处理，导致元数据被错误重建。新版本改为 `split("---", 1)`，正确识别 `---` 为 Markdown 分隔线，frontmatter 始终保留。

**Q: 先分析后翻译，分析内容"吃掉"了译文？**

A: v0.4.2 已修复。旧版本在做翻译时读取的是已包含分析内容的文件，把 `>` 开头的分析行也当成了原文来分配译文。新版本通过 `original_subtitle` 参数传入原始纯字幕内容，翻译合并时不受文件中已有分析内容干扰。
