...
| **AI 翻译** | DeepSeek V4 Flash | 英中双语字幕翻译（带编号对齐 + 强制行数修复） |
...
**Q: 下载结果表格中的文件名无法 Ctrl+点击跳转 Obsidian？**

A: 这是已知问题（v0.4.x）。当前版本通过 Rich 库的 `Text` 链接生成 OSC 8 超链接，但在部分终端环境（如 VS Code 内置终端、某些 Windows Terminal 版本）中可能无法识别。临时解决方案：在 Obsidian 中手动打开对应文件。后续版本计划直接输出 `obsidian://` URI 文本，或提供 PowerShell 辅助脚本确保兼容性。
...
A: 程序会自动删除 Windows 文件名保留字符（`\/:*?"<>|`）。文件名最大长度限制为 150 字符。
